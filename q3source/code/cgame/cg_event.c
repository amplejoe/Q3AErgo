...
// [ERGO MOD START]
qboolean isWeaponBetter(weapon_t weapon1, weapon_t weapon2)
{
	int currentWeaponIndex;
	int weaponIndex;
	int i;
	
	for (i = 0; i < NUM_HIERARCHY_WEAPONS; i++)
	{
		if (weapon1 == cg_weaponHierarchy[i]) currentWeaponIndex = i;
		if (weapon2 == cg_weaponHierarchy[i]) weaponIndex = i;
	}

	if (currentWeaponIndex < weaponIndex) return qtrue;
	else return qfalse;
}
// [ERGO MOD END]
...
================
*/
static void CG_ItemPickup(entityState_t	*es, int itemNum) { // [ERGO MOD] added es
	...
	if ( bg_itemlist[itemNum].giType == IT_WEAPON ) {
		...
		if ( cg_autoswitch.integer && bg_itemlist[itemNum].giTag != WP_MACHINEGUN ) {
			...
			// [ERGO MOD START]
			// smarter autoweapon switch (to disable completely turn off in cfg file)
			if (!isWeaponBetter(es->weapon, bg_itemlist[itemNum].giTag)) return;
			// [ERGO MOD END]
...
//		CG_Printf( "EV_JUMP_PAD w/effect #%i\n", es->eventParm );
	...
	case EV_ITEM_PICKUP:
		DEBUGNAME("EV_ITEM_PICKUP");
		{
			...
			if ( es->number == cg.snap->ps.clientNum ) {
				CG_ItemPickup(es, index); // [ERGO MOD] added es
			}
		}
		break;
