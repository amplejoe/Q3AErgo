...
code
proc G_SpawnGEntityFromSpawnVars 52 12
line 408
;396:
;397:
;398:
...
;473:
;474:	// [ERGO MOD START]	
;475:	// if worlddrops are disabled prevent certain items from spawning
;476:	if (!trap_Cvar_VariableIntegerValue("worlddrops"))
ADDRGP4 $238
ARGP4
ADDRLP4 36
...
;478:		gitem_t	*item;
;479:
;480:		// check item spawn functions
;481:		for (item = bg_itemlist + 1; item->classname; item++) {
ADDRLP4 40
ADDRGP4 bg_itemlist+52
ASGNP4
ADDRGP4 $242
...
LABELV $239
line 482
;482:			if (!strcmp(item->classname, ent->classname)) {
ADDRLP4 40
INDIRP4
INDIRP4
ARGP4
...
NEI4 $244
line 484
;483:				// do not prevent everything from spawning (flags etc. must spawn)
;484:				if (item->giType == IT_WEAPON || item->giType == IT_AMMO || item->giType == IT_ARMOR || 
ADDRLP4 48
ADDRLP4 40
INDIRP4
CNSTI4 36
...
LABELV $252
line 486
;485:					item->giType == IT_HEALTH || item->giType == IT_HOLDABLE || item->giType == IT_POWERUP)
;486:				{					
line 487
;487:					G_FreeEntity(ent); // delete entity
ADDRLP4 4
INDIRP4
ARGP4
ADDRGP4 G_FreeEntity
...
;493:	// [ERGO MOD END]
;494:
;495:	// if we didn't get a classname, don't bother spawning anything
;496:	if ( !G_CallSpawn( ent ) ) {
ADDRLP4 4
INDIRP4
ARGP4
ADDRLP4 40
...
;499:}
LABELV $189
endproc G_SpawnGEntityFromSpawnVars 52 12
...
;503:/*
;504:====================
;505:G_AddSpawnVarToken
;506:====================
;507:*/
;508:char *G_AddSpawnVarToken( const char *string ) {
line 512
;509:	int		l;
;510:	char	*dest;
;511:
...
ASGNI4
line 513
;513:	if ( level.numSpawnVarChars + l + 1 > MAX_SPAWN_VARS_CHARS ) {
ADDRGP4 level+5024
INDIRI4
ADDRLP4 0
INDIRI4
...
LEI4 $256
line 514
;514:		G_Error( "G_AddSpawnVarToken: MAX_SPAWN_CHARS" );
ADDRGP4 $259
ARGP4
ADDRGP4 G_Error
CALLV
...
line 517
;516:
;517:	dest = level.spawnVarChars + level.numSpawnVarChars;
ADDRLP4 4
ADDRGP4 level+5024
INDIRI4
ADDRGP4 level+5028
...
line 520
;519:
;520:	level.numSpawnVarChars += l + 1;
ADDRLP4 12
ADDRGP4 level+5024
ASGNP4
ADDRLP4 12
...
;525:/*
;526:====================
;527:G_ParseSpawnVars
;528:
;529:Parses a brace bounded set of key / value pairs out of the
;530:level's entity strings into level.spawnVars[]
;531:
;532:This does not actually spawn an entity.
;533:====================
;534:*/
;535:qboolean G_ParseSpawnVars( void ) {
line 539
;536:	char		keyname[MAX_TOKEN_CHARS];
;537:	char		com_token[MAX_TOKEN_CHARS];
;538:
;539:	level.numSpawnVars = 0;
ADDRGP4 level+4508
CNSTI4 0
ASGNI4
line 540
;540:	level.numSpawnVarChars = 0;
ADDRGP4 level+5024
CNSTI4 0
ASGNI4
line 543
;541:
;542:	// parse the opening brace
;543:	if ( !trap_GetEntityToken( com_token, sizeof( com_token ) ) ) {
ADDRLP4 0
ARGP4
CNSTI4 1024
ARGI4
...
EQI4 $272
line 548
;548:		G_Error( "G_ParseSpawnVars: found %s when expecting {",com_token );
ADDRGP4 $270
ARGP4
ADDRLP4 0
ARGP4
...
line 552
;550:
;551:	// go through all the key / value pairs
;552:	while ( 1 ) {	
line 554
;553:		// parse key
;554:		if ( !trap_GetEntityToken( keyname, sizeof( keyname ) ) ) {
ADDRLP4 1024
ARGP4
CNSTI4 1024
ARGI4
...
NEI4 $274
line 555
;555:			G_Error( "G_ParseSpawnVars: EOF without closing brace" );
ADDRGP4 $276
ARGP4
ADDRGP4 G_Error
CALLV
...
;561:		
;562:		// parse value	
;563:		if ( !trap_GetEntityToken( com_token, sizeof( com_token ) ) ) {
ADDRLP4 0
ARGP4
CNSTI4 1024
ARGI4
...
NEI4 $281
line 568
;568:			G_Error( "G_ParseSpawnVars: closing brace without data" );
ADDRGP4 $283
ARGP4
ADDRGP4 G_Error
CALLV
...
LABELV $281
line 570
;570:		if ( level.numSpawnVars == MAX_SPAWN_VARS ) {
ADDRGP4 level+4508
INDIRI4
CNSTI4 64
NEI4 $284
line 571
;571:			G_Error( "G_ParseSpawnVars: MAX_SPAWN_VARS" );
ADDRGP4 $287
ARGP4
ADDRGP4 G_Error
CALLV
...
LABELV $284
line 573
;573:		level.spawnVars[ level.numSpawnVars ][0] = G_AddSpawnVarToken( keyname );
ADDRLP4 1024
ARGP4
ADDRLP4 2060
...
ASGNP4
line 574
;574:		level.spawnVars[ level.numSpawnVars ][1] = G_AddSpawnVarToken( com_token );
ADDRLP4 0
ARGP4
ADDRLP4 2064
ADDRGP4 G_AddSpawnVarToken
...
;581:
;582:
;583:/*QUAKED worldspawn (0 0 0) ?
;584:
;585:Every map should have exactly one worldspawn.
;586:"music"		music wav file
;587:"gravity"	800 is default gravity
;588:"message"	Text to print during connection process
;589:*/
;590:void SP_worldspawn( void ) {
line 593
;591:	char	*s;
;592:
;593:	G_SpawnString( "classname", "", &s );
ADDRGP4 $72
ARGP4
ADDRGP4 $295
ARGP4
...
EQI4 $296
line 595
;595:		G_Error( "SP_worldspawn: The first entity isn't 'worldspawn'" );
ADDRGP4 $299
ARGP4
ADDRGP4 G_Error
CALLV
...
line 599
;597:
;598:	// make some data visible to connecting client
;599:	trap_SetConfigstring( CS_GAME_VERSION, GAME_VERSION );
CNSTI4 20
ARGI4
ADDRGP4 $300
ARGP4
...
line 601
;600:
;601:	trap_SetConfigstring( CS_LEVEL_START_TIME, va("%i", level.startTime ) );
ADDRGP4 $301
ARGP4
ADDRGP4 level+40
INDIRI4
...
pop
line 604
;604:	trap_SetConfigstring( CS_MUSIC, s );
CNSTI4 2
ARGI4
ADDRLP4 0
INDIRP4
...
pop
line 607
;607:	trap_SetConfigstring( CS_MESSAGE, s );				// map specific message
CNSTI4 3
ARGI4
ADDRLP4 0
INDIRP4
...
line 609
;608:
;609:	trap_SetConfigstring( CS_MOTD, g_motd.string );		// message of the day
CNSTI4 4
ARGI4
ADDRGP4 g_motd+16
ARGP4
...
line 611
;610:
;611:	G_SpawnString( "gravity", "800", &s );
ADDRGP4 $305
ARGP4
ADDRGP4 $306
ARGP4
...
line 614
;613:
;614:	G_SpawnString( "enableDust", "0", &s );
ADDRGP4 $308
ARGP4
ADDRGP4 $211
ARGP4
...
pop
line 615
;615:	trap_Cvar_Set( "g_enableDust", s );
ADDRGP4 $309
ARGP4
ADDRLP4 0
INDIRP4
...
line 617
;616:
;617:	G_SpawnString( "enableBreath", "0", &s );
ADDRGP4 $310
ARGP4
ADDRGP4 $211
ARGP4
...
pop
line 618
;618:	trap_Cvar_Set( "g_enableBreath", s );
ADDRGP4 $311
ARGP4
ADDRLP4 0
INDIRP4
...
line 620
;619:
;620:	g_entities[ENTITYNUM_WORLD].s.number = ENTITYNUM_WORLD;
ADDRGP4 g_entities+825776
CNSTI4 1022
ASGNI4
line 621
;621:	g_entities[ENTITYNUM_WORLD].classname = "worldspawn";
ADDRGP4 g_entities+825776+524
ADDRGP4 $298
ASGNP4
line 624
;622:
;623:	// see if we want a warmup time
;624:	trap_SetConfigstring( CS_WARMUP, "" );
CNSTI4 5
ARGI4
ADDRGP4 $295
ARGP4
...
pop
line 625
;625:	if ( g_restarted.integer ) {
ADDRGP4 g_restarted+12
INDIRI4
CNSTI4 0
EQI4 $315
line 626
;626:		trap_Cvar_Set( "g_restarted", "0" );
ADDRGP4 $318
ARGP4
ADDRGP4 $211
ARGP4
...
pop
line 627
;627:		level.warmupTime = 0;
ADDRGP4 level+16
CNSTI4 0
ASGNI4
line 628
;628:	} else if ( g_doWarmup.integer ) { // Turn it on
ADDRGP4 $316
JUMPV
LABELV $315
...
EQI4 $320
line 629
;629:		level.warmupTime = -1;
ADDRGP4 level+16
CNSTI4 -1
ASGNI4
line 630
;630:		trap_SetConfigstring( CS_WARMUP, va("%i", level.warmupTime) );
ADDRGP4 $301
ARGP4
ADDRGP4 level+16
INDIRI4
...
;637:/*
;638:==============
;639:G_SpawnEntitiesFromString
;640:
;641:Parses textual entity definitions out of an entstring and spawns gentities.
;642:==============
;643:*/
;644:void G_SpawnEntitiesFromString( void ) {
line 646
;645:	// allow calls to G_Spawn*()
;646:	level.spawning = qtrue;
ADDRGP4 level+4504
CNSTI4 1
ASGNI4
line 647
;647:	level.numSpawnVars = 0;
ADDRGP4 level+4508
CNSTI4 0
ASGNI4
line 652
;648:
;649:	// the worldspawn is not an actual entity, but it still
;650:	// has a "spawn" function to perform any global setup
;651:	// needed by a level (setting configstrings or cvars, etc)
;652:	if ( !G_ParseSpawnVars() ) {
ADDRLP4 0
...
NEI4 $329
line 653
;653:		G_Error( "SpawnEntities: no entities" );
ADDRGP4 $331
ARGP4
ADDRGP4 G_Error
CALLV
...
;656:
;657:	// parse ents
;658:	while( G_ParseSpawnVars() ) {
line 659
;659:		G_SpawnGEntityFromSpawnVars();
...
line 662
;661:
;662:	level.spawning = qfalse;			// any future calls to G_Spawn*() will be errors
ADDRGP4 level+4504
CNSTI4 0
ASGNI4
line 663
